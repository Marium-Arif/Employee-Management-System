...
router.post('/add_task', (req, res) => {
    ...
    (task_name,task_description,due_date, status, project_id, employee_id) 
    VALUES (?)`;
    const values = [
        req.body.task_name,
...
router.put('/edit_project/:p_id', (req, res) => {
    ...
    con.query(sql,[...values, p_id], (err, result) => {
        if(err) return res.json({Status: false, Error: "Query Error"+err})
        return res.json({Status: true, Result: result})
    })
});
...
router.delete('/delete_project/:p_id', (req, res) => {
    const p_id = req.params.p_id;
    const sql = "delete from  Project where p_id = ?"
